...
#define FILE_OPT_CFG_DIR "cfg"  //用户配置文件路径
#define FILE_OPT_CFG_NAME "cloudFile.cfg"   //云文件配置文件
...
typedef struct STRUCT_PARA_TABLE_ELEMENTtmp{
                ...
                char new_file_name[PARA_FILE_NAME_MAX_SIZE];//新文件名
...
public:
    ...
    QString current_user_cfg_path;  //当前用户配置文件路径
    QString current_user_cfg_file;  //当前用户配置文件

    void test();
    void init(QString wkp);
    int manage(int opt, QString name, QByteArray content, QString new_name);
    int manageAndSave(int opt, QString name, QByteArray content, QString new_name);
    ...
    int userFloderInit(QString wkp, QString user_name);
    int setCurrentUserFloder(QString wkp, QString user_name);
    ...
    int syncFile();
    int renameFile(QString name, QString new_name);
