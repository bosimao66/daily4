...
FileOpt::FileOpt(QString wkpath)
{
   //qDebug()<<"FileOpt::FileOpt(QString wkpath) is comming";
   init(wkpath);
   //setCurrentUserFloder();
}
...
void FileOpt::init(QString wkp)
{
    ...
    //创建备份目录
    back_path.clear();
    back_path.sprintf("%s/%s", wk_path.toLatin1().data(), FILE_OPT_BACK_DIR);
    d.mkdir(back_path);

   //创建仓库目录
    store_path.clear();
    store_path.sprintf("%s/%s", wk_path.toLatin1().data(), FILE_OPT_STORE_DIR);
    d.mkdir(store_path);

     //创建用户目录
    user_path.clear();
    user_path.sprintf("%s/%s", wk_path.toLatin1().data(), FILE_OPT_USER_DIR);
    d.mkdir(user_path);
...
//#####################################################功能函数
int FileOpt::userFloderInit(QString wkp, QString user_name)//用户目录初始化化  规定path尾部不带 /
{
    QString tmp1, tmp2, tmp3, tmp4;
    tmp1.clear();
    tmp1.sprintf("%s/%s/%s", wkp.toLatin1().data(), FILE_OPT_USER_DIR, user_name.toLatin1().data());
    tmp2.clear();
    tmp2.sprintf("%s/%s", wkp.toLatin1().data(), FILE_OPT_USER_DIR);
    tmp3.clear();
    tmp3.sprintf("%s/%s/%s/%s", wkp.toLatin1().data(), FILE_OPT_USER_DIR, user_name.toLatin1().data(), FILE_OPT_USER_TABLE_DIR);
    QDir d;
    //qDebug()<<"userFloderInit: tmp1="<<tmp1<<"  tmp2="<<tmp2<< "  tmp3="<<tmp3;
    QByteArray block;
    ...
    if(true == d.cd(tmp2))
    {
        if(floderExist(tmp1) == false)
        {
            d.mkdir(user_name);
            new_mark = 1;
        }

        if(true == d.cd (tmp1) && new_mark == 1)//进入到用户目录
        {
            ...
            //创建配置目录 和配置文件
            d.mkdir(FILE_OPT_CFG_DIR);
            tmp4.clear();
            tmp4.append(tmp1);
            tmp4.append("/");
            tmp4.append(FILE_OPT_CFG_DIR);
            tmp4.append("/");
            tmp4.append(FILE_OPT_CFG_NAME);
            f.setFileName(tmp4);
            if(f.exists() == false)
            {
                f.open(QIODevice::WriteOnly | QIODevice::Truncate);
...
int FileOpt::setCurrentUserFloder(QString wkp, QString user_name)//设置当前的用户目录
{
    current_user_name.clear();
    current_user_name.append(user_name);

    current_user_path.clear();
    current_user_path.sprintf("%s/%s/%s", wkp.toLatin1().data(), FILE_OPT_USER_DIR, user_name.toLatin1().data());
    current_user_table_path.clear();
    current_user_table_path.sprintf("%s/%s/%s/%s", wkp.toLatin1().data(), FILE_OPT_USER_DIR, user_name.toLatin1().data(), FILE_OPT_USER_TABLE_DIR);
    ...
    current_user_cfg_path.clear();
    current_user_cfg_path.sprintf("%s/%s", current_user_path.toLatin1().data(), FILE_OPT_CFG_DIR);  //当前用户配置文件路径
    current_user_cfg_file.clear();
    current_user_cfg_file.sprintf("%s/%s", current_user_cfg_path.toLatin1().data(), FILE_OPT_CFG_NAME);  //当前用户配置文件
    return 0;
}
...
int FileOpt::linkToParaFile(OptLink *head, QString file_name)
{
    ...
    while(prob != NULL)
    {
        ...
        strcpy(ele.file_name, prob->obj.toUtf8());
        strcpy(ele.new_file_name, prob->new_name.toUtf8());
        ele.index = index;
        ele.opt = prob->opt;
...
OptLink *FileOpt::fileToLink(QString para_name, QString data_name)
{
    ...
    for(i = 0; i < num; i++)
    {
        ...
        node->new_name.clear();
        node->new_name.append(ele.new_file_name);
...
//文件操作，并存于链表
int FileOpt::manageAndSave(int opt, QString name, QByteArray content, QString new_name)
{
    int ret;
    ret = manage(opt, name, content, new_name);
...
int FileOpt::linkNodeAction(OptLink *node)
{
    ...
    tmp.append("/");
    tmp.append(node->obj);
    return manage(node->opt, tmp, node->content, node->new_name);
}
...
int FileOpt::linkBackAction(OptLink *node)
{
    ...
    {
        //新建的回退动作是删除
        ret = manage(fileOptCode_del, tmp, node->back, node->new_name);
        //opt_link_int->linkDelNode_byIndex();
    }
    else if(node != NULL && node->opt == fileOptCode_cover)
    {
        //覆盖的回退动作还是覆盖
        ret = manage(fileOptCode_cover, tmp, node->back, node->new_name);

    }
    else if(node != NULL && node->opt == fileOptCode_del)
    {
        //删除的动作是新建
        ret = manage(fileOptCode_new, tmp, node->back, node->new_name);
    }
    else if(node != NULL && node->opt == fileOptCode_rename)
    {
        //重命名的动作是重命名
        ret = manage(fileOptCode_rename, node->new_name, node->back, tmp);
    }
    else
    {
        return -1;
...
//文件操作管理
int FileOpt::manage(int opt, QString name, QByteArray content, QString new_name)
{
    int ret;
    ...
    else if(opt == fileOptCode_rename)
    {
        ret = renameFile(name, new_name);
    }
    return ret;
}
...
//重命名文件
int FileOpt::renameFile(QString name, QString new_name)
{
    QFile f(name);
    if(true == f.rename(new_name))
    {
        return 0;
    }
    return -1;
