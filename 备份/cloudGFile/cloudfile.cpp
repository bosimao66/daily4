...
//	//文件操作接口初始化
//	file_opt_int = new FileOpt();
//	file_opt_int->setOptLinkHead(opt_link_head, opt_link_int);
//    //默认用户名
//    current_user_name.clear();
//    current_user_name.append("guest");
//    curent_user_passwd.clear();
//    char tmp1[8] = "123";
//    char tmp2[32];
//    dataEncrypt_byte8(tmp1, 3, tmp2);
//    curent_user_passwd.append(tmp2);
//    user_used_mark = 1;
...
void cloudFile::testFun()
{
    ...
    file_opt_int->userFloderInit("guest");
...
void cloudFile::testFun2()
{
    ...
    file_opt_int->setCurrentUserFloder("guest");
...
int cloudFile::replyAccountLogin(STRUCT_DATA_HEAD head)
{
    if(head.paraInt[0] == 1)
    {
        qDebug()<<"replyAccountRegister: login successfully.";
    }
    else if(head.paraInt[0] == -1)
    {
        qDebug()<<"replyAccountRegister: login is faild, user name is not exist.";
    }
    else if(head.paraInt[0] == -2)
    {
        qDebug()<<"replyAccountRegister: login is faild, password error.";
    }
    return 0;
}
//回复处理：用户销毁
...
void cloudFile::sendData(QTcpSocket *socket, STRUCT_DATA_HEAD head, QByteArray data)
{
    ...
    if(user_used_mark == 1)
    {
        ...
        strcpy(head.passwd, curent_user_passwd.toLatin1().data());
    }
